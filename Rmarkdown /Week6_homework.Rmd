...
plotTides= function(df){
  ...
  plot(x = df$TimeUTC, y = df$TideHT, type = 'l',
       xlab= "Dates", ylab= "Tide Height, meters", col = 'red') 
  
}
```


``` {r plotJan, cache= TRUE}
Jan= NOAAFunction(station= 9413450, startDate = 20170101, endDate = 20170131 )

plotTides(Jan)

```

January 01 - January 31, 2017 

``` {r plotFeb, cache= TRUE}
Feb= NOAAFunction(station= 9413450, startDate = 20170201, endDate = 20170228 )

plotTides(Feb)
```

Feburary 01 - Feburary 28, 2017 

``` {r plotMar, cache= TRUE}
Mar= NOAAFunction(station= 9413450, startDate = 20170301, endDate = 20170330 )

plotTides(Mar)

```

March 01 - March 30, 2017 
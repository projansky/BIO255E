...
dataframe= function(plate1){
  ...
  # Plot the Growth column vs Date column
  # Split for each separate LimpetID
  
for(i in 1:nrow(longdata) ) {
  
  if(i == 1){
    plot(x= longdata$Date, y= longdata$Growth, col= i)
    text(x= longdata$Date, y= longdata$Growth, 
       labels = longdata[, "Tag"],
       cex=0.7, pos = 4)
  } else {
    points(x= longdata$Date, y= longdata$Growth, col=i+1)
    unique(text(x= longdata$Date, y= longdata$Growth, 
       labels = longdata[, "Tag"],
       cex=0.7, pos = 4))
  }
}  

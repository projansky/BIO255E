...
one.plate= function(plate, month1, month2){
  ...
  # Plot the difference between two months 
  # ±5 is the "acceptable" growth/loss for a limpet 
...
## Plate 10: 
  * B78 looks like the same mussel and may have grown an accurate ammount from the months of August to October 
...
## Five Plate Function 
Similar to the one plate function and the three plate function to plot the difference in limpet growth, this function does the same with five total plates. The function is very similar to the three plate with the addition of two more plates being plotted. A legend is created the same way and the function is best run when 5 plates are in a row (1, 2, 3, 4, 5). 
``` {r five_plate_growth, echo= TRUE}
   ...
   # Go through the data frame. If the Number column is 1, then make a plot
    ...
    if(i == 1){
      plot(x= longdata$Date[longdata$Num == i], 
           ...
           main= paste("Six Month Limpet Tracker of Plate", plate1) ) 
